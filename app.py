...
[매우 중요: 공백 제외 30자 이상 시 무조건 강제 줄바꿈 및 자막 슬래시 포맷 규칙 (ABSOLUTE CRITICAL)]
...
3. [가이드 필수 내용 100% 반영 & 브랜드명 조건부 발화]:
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [모든 텍스트 영역에서 공백 제외 30자 이상 시 무조건 강제 줄바꿈], [자막 서술형 절대 금지 및 명사형 단문], [자막 포맷: 한 줄 띄우고 / 슬래시 단독 줄 배치 후 한 줄 띄우기], [나레이션 내 실제 브랜드명 {brand_name} 발화 시에만 브랜드+제품+키워드 자막 표기], [나레이션 공백포함 280자 이내], [출력물 내 프롬프트 지시어 삭제], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
