...
[매우 중요: 공백 제외 30자 이상 시 강제 줄바꿈, 자막 슬래시 포맷, 비포애프터 규칙 (ABSOLUTE CRITICAL)]
1. [30자 초과 시 강제 줄바꿈 (절대 규칙)]:
- **썸네일 설명, 씬별 촬영 컷 설명, 자막, 나레이션, 하단 구성 요소(주의점, 필수 표기 사항, 로고 안내, 심의 안내), 광고 캡션, 해시태그, 썸네일 재요약, 장면 요약, 나레이션만 정리 등 대본의 모든 텍스트 영역에서 공백을 제외한 글자 수가 30자 이상이 되면 무조건 엔터(줄바꿈)`\n`을 쳐서 세로로 쪼개어 출력하세요.**
- 단 한 줄이라도 30자 이상 길게 작성하여 브라우저 화면에 가로 스크롤이 생기게 하는 행위는 엄격히 금지합니다.

2. [자막 서술형 절대 금지 및 슬래시 포맷 (핵심 규칙)]:
- 자막에는 절대 서술형(~해라, ~해보세요 등)을 쓰지 말고, 오직 명사형/요약형 단문 및 키워드만 사용하세요.
...
3. [비포애프터(B/A) 자막 강제 삽입 규칙 (매우 중요)]:
- **장면 설명(연출 내용)에 비포, 애프터, B/A, 비교 등의 키워드가 포함되어 있다면, 해당 장면의 자막 영역에 반드시 독립된 줄로 `(비포애프터)`라는 한글 표기를 포함하세요.**

4. [가이드 필수 내용 100% 반영 & 브랜드명 조건부 발화]:
- 가이드라인 필수 항목을 100% 반영하고, 나레이션에 실제 브랜드명({brand_name})이 직접 발화될 때만 자막에 브랜드명(로고 삽입) 형태로 표기하세요.
- 전체 나레이션 총합 분량은 공백 포함 280자 이내 엄수. 기울임꼴 금지.
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [모든 텍스트 영역에서 공백 제외 30자 이상 시 무조건 강제 줄바꿈], [자막 서술형 절대 금지 및 명사형 단문], [장면 설명에 비포/애프터/B/A 포함 시 자막에 반드시 (비포애프터) 단독 줄 추가], [자막 포맷: 한 줄 띄우고 / 슬래시 단독 줄 배치 후 한 줄 띄우기], [나레이션 내 실제 브랜드명 {brand_name} 발화 시에만 브랜드+제품+키워드 자막 표기], [나레이션 공백포함 280자 이내], [출력물 내 프롬프트 지시어 삭제], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
