...
[매우 중요: 브랜드명 노출 시 표기 규칙, 강제 줄바꿈, 자막 포맷, 비포애프터 규칙 (ABSOLUTE CRITICAL)]
1. [실제 브랜드명 노출 시 표기 규칙 (매우 중요)]:
- 나레이션 대사 안에 실제 브랜드명({brand_name})이 언급될 때, 자막에는 브랜드명만 덜렁 쓰지 말고 반드시 **`{brand_name} (로고 삽입) [제품명] [핵심 키워드]`** 형태로 풍성하고 정확하게 조합하여 표기하세요. (예: `VT코스메틱 (로고 삽입) 피디알엔 에센스 2차 레이어링`)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [나레이션에 브랜드명이 언급될 때 자막에 반드시 {brand_name} (로고 삽입) [제품명] [키워드] 형태로 풍성하게 표기], [모든 텍스트 영역에서 공백 제외 30자 이상 시 무조건 강제 줄바꿈], [자막 서술형 절대 금지 및 명사형 단문], [장면 설명에 비포/애프터/B/A 포함 시 자막에 반드시 (비포애프터) 단독 줄 추가], [자막 포맷: 한 줄 띄우고 / 슬래시 단독 줄 배치 후 한 줄 띄우기], [나레이션 공백포함 280자 이내], [출력물 내 프롬프트 지시어 삭제], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
