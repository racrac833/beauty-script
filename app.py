...
[매우 중요: 인스타그램 대본 작성 핵심 규칙 (CRITICAL)]
1. [가이드 필수 내용 100% 반영]:
- 가이드라인에 명시된 필수 장면, 필수 자막, 필수 나레이션, 필수 안내 사항은 절대 누락하지 말고 100% 반영하세요.

2. [브랜드명 표기 규칙 엄수 (핵심)]:
- 대괄호 형태의 플레이스홀더(`[브랜드명]`, `[제품명]`)를 절대 쓰지 마세요.
- **나레이션 대사 안에 실제 브랜드명({brand_name})이 명시적으로 직접 발화되어 언급될 때만** 자막에 `{brand_name}(로고 삽입) {product_name} [명사형 키워드]` 형태로 표기합니다.
- 나레이션에 브랜드명이 나오지 않는데 자막에 강제로 브랜드명을 넣는 행위는 **절대 금지**입니다.
- 브랜드명 표기 시 '사용'이라는 단어는 절대 쓰지 말고 제품 특성에 맞는 명사형 단어(예: 진정 케어, 수분 충전 등)로 대체하세요.

3. [비포&애프터 자막 규칙]:
- 장면 설명에 피부 변화나 비포/애프터 비교 내용이 포함된 경우, 자막에 반드시 `비포 애프터` 문구를 포함하세요.

4. [나레이션 분량 단축]:
- 전체 나레이션 총합 분량은 **공백 포함 280자 이내**로 타이트하게 작성하세요.

5. [자막 슬래시 구분선 및 동적 확장 규칙 (매우 중요)]:
- 자막은 절대 한 줄로 길게 쓰지 마세요.
- 자막의 줄과 줄 사이에는 반드시 아래 형식처럼 **슬래시(`/`)를 단독 줄로 배치**하여 명확히 구분하세요:
  [자막 첫 줄 내용]
  /
  [자막 둘째 줄 내용]
- 나레이션의 호흡과 분량이 길어질 경우 자막을 1개만 쓰지 말고, 분량에 맞춰 자막 묶음을 동적으로 늘려 여러 세트로 배치하세요.

6. [가로 스크롤 방지 및 강제 줄바꿈 (CRITICAL)]:
- 모든 텍스트(썸네일, 앵글 설명, 자막, 나레이션, 하단 캡션 등)는 절대 한 줄로 길게 쓰지 말고, 20~30자 내외마다 **반드시 엔터(줄바꿈)**를 쳐서 브라우저 화면에서 가로 스크롤이 절대 생기지 않게 하세요. (기울임꼴 금지)
...
자막：
(나레이션에 브랜드명 언급 시에만 브랜드명(로고 삽입) 제품명 키워드 표기 / 
두 줄 이상 시 반드시 아래처럼 슬래시를 단독 줄로 배치 / 
줄바꿈 필수)
자막 첫 번째 줄 문장
/
자막 두 번째 줄 문장
...
(가이드 요구사항에 맞춘 최적의 씬 개수만큼 '### [2. 장면]' 형식으로 순차적 반복)
...
나레이션만 정리
(전체 나레이션을 모아서 문장 단위로 줄바꿈 출력 / 
줄바꿈 필수)
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [나레이션 내 실제 브랜드명 {brand_name} 발화 시에만 브랜드+제품+키워드 자막 표기 및 '사용' 단어 금지], [비포 애프터 자막 적용], [나레이션 공백포함 280자 이내], [자막 줄 사이에 / 슬래시 단독 줄 배치 및 동적 묶음 확장], [추천 썸네일 띄어쓰기 철저 준수], [출력물 내 프롬프트 지시어 삭제], [가로 스크롤 방지 강제 줄바꿈 필수], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
...
[매우 중요: 가이드 필수 항목 반영 및 엄격한 규칙 (CRITICAL)]
1. [가이드 필수 내용 100% 반영]:
- 제공된 가이드라인에 명시된 필수 사진 컷, 필수 문구, 필수 해시태그, 필수 안내 사항은 절대 누락하지 말고 원고 구석구석에 반드시 녹여내어 반영하세요.

2. [실제 브랜드명 및 제품명 적용]:
- 원고 내에 대괄호 형태의 플레이스홀더를 절대 쓰지 말고, 입력된 **실제 브랜드명({brand_name})**과 **실제 제품명({product_name})**을 정확히 기재하세요.

3. [출력물 내 프롬프트 지시어 노출 금지 및 강제 줄바꿈]:
- 원고 결과물 내에 시스템 지시어 문구를 절대 출력하지 마세요.
- 촬영 가이드, 본문 원고, 제목 등 모든 텍스트는 25~35자 내외마다 엔터(줄바꿈)를 쳐서 가로 스크롤이 절대 생기지 않게 하세요.

4. [필수 해시태그 세로 줄바꿈 출력]:
- 최하단 [필수 해시태그] 영역에서는 지정된 해시태그들을 한 줄에 길게 쓰지 말고, 각 해시태그마다 반드시 엔터를 쳐서 세로로 한 줄씩 나열하세요.

5. [블로그 SEO 및 분량 기준]:
- 사진 장수는 가이드라인에 맞춰 최적의 개수로 [사진 1]부터 순차 구성.
- 본문 총분량은 공백 제외 1,500자 ~ 2,000자 사이 엄수. 제목은 공백 포함 25~35자 내외 5선 추천. '시술' 등 의료 표현 금지. ~해봤는데요 톤 유지.

[출력 양식 템플릿]
[네이버 블로그 추천 제목 5선 (SEO 최적 글자수 25~35자)]
1. ({brand_name} 및 키워드 배치 제목)
2. 
3. 
4. 
5. 
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 사진/문구/내용 100% 누락 없이 반영], [실제 브랜드명 {brand_name} 및 제품명 {product_name} 적용 (대괄호 금지)], [필수 해시태그 세로 줄바꿈 나열], [출력물 내 프롬프트 지시어 일체 삭제], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [가로 스크롤 방지 강제 줄바꿈], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
