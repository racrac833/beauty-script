...
[매우 중요: 블로그 최적화 사진 15~20장 및 사진별 1:1 단락 반복 & 맨 하단 촬영가이드 총정리 규칙 (ABSOLUTE CRITICAL)]
1. [기계식 인사말 절대 금지]:
...
[본문 원고]
(이 사진에 해당하는 상세 찐후기 원고 / 공백 제외 30자 이상 시 엔터 줄바꿈)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [기계식 인사말 절대 금지 및 자연스러운 후기 톤], [블로그 추천 제목 5선 포함], [사진별 촬영가이드 + 본문원고 1:1 쌍 단락을 15~20장 반복], [본문 원고는 공백 제외 30자 이상 시 무조건 엔터 쳐서 줄바꿈], [맨 하단에 사진 촬영 가이드 총정리 리스트 배치], [마크다운 별표(**) 사용 금지], [해시태그 무조건 세로로 한 줄씩 나열], [가이드 필수 내용 100% 반영], [공백 제외 글자수 1500~2000자 내외], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
