...
5. [캡션 및 본문 문장 단위 필수 줄바꿈 (매우 중요)]:
- **[#광고 캡션] 영역을 작성할 때 절대 한 문단으로 길게 뭉쳐서 쓰지 마세요.** 
- 모든 문장은 마침표나 느낌표 단위로 **반드시 엔터(줄바꿈)를 쳐서** 읽기 편하게 여러 줄로 나누어 작성하세요. 가로 스크롤이나 통짜 텍스트 뭉치는 절대 금지입니다.
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [썸네일은 첫줄 공백 제외 10자 이내, 둘째줄 공백 제외 12자 이내 엄수], [나레이션에 브랜드명/제품명 언급 시 자막에 {brand_name} (로고 삽입) {product_name} 형태를 절대 쪼개지 말고 온전한 덩어리로 표기], [자막은 나레이션을 요약한 명사형 단문], [캡션은 문장마다 반드시 엔터를 쳐서 여러 줄로 분할 작성], [자막 서술형 절대 금지 및 명사형 단문], [장면 설명에 비포/애프터/B/A 포함 시 자막에 반드시 (비포애프터) 단독 줄 추가], [자막 포맷: 한 줄 띄우고 / 슬래시 단독 줄 배치 후 한 줄 띄우기], [나레이션 공백포함 280자 이내], [출력물 내 프롬프트 지시어 삭제], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
...
[매우 중요: 문장 단위 필수 줄바꿈 (ABSOLUTE CRITICAL)]
1. [문장 단위 줄바꿈]:
- 제목, 촬영 가이드, 본문 원고 텍스트 등 블로그 원고의 모든 텍스트 영역에서 문장마다 반드시 엔터(줄바꿈)를 쳐서 가독성을 높이세요. 한 문단으로 길게 뭉쳐 쓰는 것을 엄격히 금지합니다.
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 사진/문구/내용 100% 누락 없이 반영], [문장마다 엔터 쳐서 줄바꿈], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
