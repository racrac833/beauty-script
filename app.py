...
def validate_and_fix_result(text, brand, product):
    ...
    for line in lines:
        ...
        if "[베스트 썸네일]" in line or "[추천 썸네일 문구 9선]" in line:
            ...
            continue
        elif "메인 스크립트" in line or "### [" in line or "[사진 1]" in line:
            in_thumbnail_section = False
...
[매우 중요: 숏폼 후킹 및 가이드 필수 장면 반영 규칙 (ABSOLUTE CRITICAL)]
1. [초반 강력한 후킹 멘트 배치 (필수)]:
- 릴스 초반(시작 장면)부터 대뜸 브랜드명이나 제품명을 먼저 읊어버리면 시청자가 광고인 줄 알고 이탈합니다.
- **초반에는 소비자가 겪는 극심한 피부 고민이나 호기심을 자극하는 강력한 반전/질문형 후킹 멘트로 시선을 사로잡은 뒤, 자연스럽게 브랜드와 제품({brand_name} {product_name})으로 전환하세요.**

2. [가이드라인 '필수 장면' 100% 반영 (필수)]:
- 첨부된 가이드라인 이미지나 텍스트 내에 **[필수 장면 1], [필수 컷] 등으로 지정된 연출 가이드나 필수 포함 사항이 있다면, 대본 내에 절대 누락하지 말고 해당 씬(장면)에 충실히 반영**하세요.

3. [부드러운 구어체 톤앤매너 (절대 규칙)]:
- 딱딱하고 기계적인 표현(~했습니다, ~합니다, ~입니다)은 절대 사용하지 마세요.
- 30대 뷰티 인플루언서가 실제로 소통하듯 부드럽고 친근한 표현으로 작성하세요.
  * ~했습니다 / ~했어요 ➔ ~해보았는데요 / ~했거든요 / ~더라고요
  * ~합니다 ➔ ~해요 / ~하더라고요 / ~거든요
  * ~입니다 ➔ ~인데요 / ~이더라고요

4. [썸네일 글자 수 및 띄어쓰기 유지 (절대 규칙)]:
...
8. [자막 슬래시 포맷 및 (비포애프터) 위치별 격리 규칙 (매우 중요)]:
- 자막은 나레이션 대사의 핵심 내용을 직관적으로 압축한 명사형 단문이어야 합니다. 서술형 절대 금지.
- 두 줄 이상 자막은 슬래시(`/`)로 위아래를 격리합니다.
- **`(비포애프터)` 또는 `(비포애프터) 설명`이 자막 단락 내에서 위치하는 곳에 따라 슬래시 규칙을 엄격히 적용하세요:**
  * **단락의 맨 처음(첫 번째 줄)에 올 때:** 위쪽에는 슬래시를 절대 넣지 않고, 아래쪽에만 슬래시(`/`)를 두어 다음 자막과 격리합니다.
  * **단락의 맨 마지막(마지막 줄)에 올 때:** 아래쪽에는 슬래시를 절대 넣지 않고, 위쪽에만 슬래시(`/`)를 두어 이전 자막과 격리합니다.
  * **단락의 중간에 올 때:** 위아래 모두 슬래시로 완벽히 격리합니다 (`/ (비포애프터) /`).

9. [마크다운 강조(**) 금지 및 30자 초과 시 강제 줄바꿈]:
...
(비포애프터 또는 위치별 슬래시 격리 규칙 준수)

나레이션：
(초반 강력한 후킹 멘트 적용 / 부드러운 구어체 찐후기 나레이션)
첫 번째 나레이션 문장
두 번째 나레이션 문장
...
#광고 캡션
(30대 여성 부드러운 찐후기 톤 / 공백 제외 30자 이상 시 반드시 엔터 줄바꿈)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드라인 필수 장면 및 필수 포함 사항 100% 누락 없이 반영], [초반 강력한 호기심 유발 후킹 멘트 적용], [딱딱한 표현(~했습니다/합니다/입니다) 절대 금지 및 30대 여성 부드러운 구어체(~했는데요/~해요/~인데요) 사용], [썸네일은 띄어쓰기 포함하여 첫줄 공백 제외 10자 이내, 둘째줄 공백 제외 12자 이내 엄수], [장면 설명은 의미 단위마다 엔터 쳐서 세로 분할], [메인 스크립트는 장면 ➔ 자막 ➔ 나레이션 순서 엄수], [나레이션에 브랜드명/제품명 언급 시 자막에 {brand_name} (로고 삽입) {product_name} 형태를 절대 쪼개지 말고 온전한 덩어리로 표기], [자막은 나레이션을 요약한 명사형 단문], [마크다운 별표(**) 사용 금지], [(비포애프터)는 단락 내 위치(처음/끝/중간)에 따라 위 또는 아래 슬래시 생략 규칙 철저 적용], [공백 제외 30자 이상 시 무조건 엔터 쳐서 줄바꿈], [해시태그는 무조건 세로로 한 줄씩 나열], [나레이션 공백포함 280자 이내], [출력물 내 프롬프트 지시어 삭제], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
...
[매우 중요: 톤앤매너 및 블로그 구성 규칙 (ABSOLUTE CRITICAL)]
1. [부드러운 구어체 톤앤매너 (절대 규칙)]:
- 딱딱하고 기계적인 표현(~했습니다, ~합니다, ~입니다)은 절대 사용하지 마세요.
- 30대 뷰티 블로거가 이웃들과 소통하듯 부드럽고 친근한 표현으로 작성하세요.
  * ~했습니다 / ~했어요 ➔ ~해보았는데요 / ~했거든요 / ~더라고요
  * ~합니다 ➔ ~해요 / ~하더라고요 / ~거든요
  * ~입니다 ➔ ~인데요 / ~이더라고요
- 기계식 인사말("안녕하세요 뷰티 인플루언서입니다") 절대 금지. 자연스러운 후기 톤으로 즉시 시작.

2. [블로그 추천 제목 5선 포함]:
- 최상단에 네이버 SEO 최적화된 추천 제목 5선(공백 포함 25~35자 내외)을 먼저 제시하세요.

3. [사진 1컷당 가이드 + 원고 1쌍 구조 반복 (15~20장)]:
- **네이버 SEO 최적화와 상위 노출을 위해 반드시 총 15장 ~ 20장 분량의 사진 컷을 구성하세요.**
- **`[사진 1]` 촬영 가이드를 먼저 쓰고, 그 바로 밑에 해당 사진에 맞는 상세 찐후기 본문 원고를 쌍으로 작성하는 단락을 사진 장수만큼 반복**하세요.

4. [본문 원고 30자 이상 시 강제 줄바꿈 & 별표(**) 금지]:
...
5. [맨 하단 사진 촬영 가이드 총정리]:
- **모든 1:1 단락 원고가 끝난 맨 하단에, 전체 사진 촬영 가이드만 다시 한 번 싹 모아서 아래 형식으로 깔끔하게 리스트업하세요.**
  [사진 촬영 가이드 총정리]
  [사진 1] : 내용
  [사진 2] : 내용
  ... (15~20장까지)
...
[본문 원고]
(이 사진에 해당하는 상세 찐후기 원고 / 부드러운 구어체 / 공백 제외 30자 이상 시 엔터 줄바꿈)
...
(동일한 구조로 [사진 3]부터 [사진 15~20]까지 가이드와 본문 원고 쌍을 반복)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [딱딱한 표현(~했습니다/합니다/입니다) 절대 금지 및 30대 여성 부드러운 구어체(~했는데요/~해요/~인데요) 사용], [블로그 추천 제목 5선 포함], [사진별 촬영가이드 + 본문원고 1:1 쌍 단락을 15~20장 반복], [본문 원고는 공백 제외 30자 이상 시 무조건 엔터 쳐서 줄바꿈], [맨 하단에 사진 촬영 가이드 총정리 리스트 배치], [마크다운 별표(**) 사용 금지], [해시태그 무조건 세로로 한 줄씩 나열], [가이드 필수 내용 100% 반영], [공백 제외 글자수 1500~2000자 내외], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
