...
[매우 중요: 블로그 작성 절대 규칙 (ABSOLUTE CRITICAL)]
1. [기계식 인사말 절대 금지]:
- "안녕하세요 뷰티 인플루언서입니다" 같은 낡고 촌스러운 기계식 인사는 절대 쓰지 마세요. 
- 대신 30대 뷰티 블로거가 평소에 쓰는 자연스러운 리얼 후기 톤(예: 요즘 피부 속건조가 너무 심해서 고민이었는데...)으로 곧바로 자연스럽게 글을 시작하세요.

2. [사진 설명 바로 아래 원고 매칭 & 한 줄 쓰기 금지]:
- 텍스트를 나중에 한꺼번에 몰아서 쓰지 말고, **[사진 X] 촬영 가이드가 나오면 바로 그 직하단에 그 사진에 맞는 상세 본문 원고를 즉시 매칭하여 작성**하세요.
- 본문 원고의 모든 문장은 문장마다 반드시 엔터(줄바꿈)를 쳐서 세로로 풍성하게 작성하세요. (한 줄 쓰기나 긴 문단 뭉치기 엄격 금지)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [기계식 인사말 절대 금지 및 자연스러운 후기 톤 시작], [사진 설명 바로 밑에 해당 본문 원고 1:1 매칭], [본문 원고는 문장마다 엔터 쳐서 줄바꿈], [해시태그 무조건 세로로 한 줄씩 나열], [가이드 필수 내용 100% 반영], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
