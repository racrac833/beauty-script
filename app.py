...
# HTML 선택 값을 파이썬 상태로 안전하게 반영하는 쿼리 핸들러
...
# ==================== [2. 테마 컬러 및 완벽 고정 크기 커스텀 박스 스타일링] ====================
...
<style>
    ...
    /* 6. [핵심 완성형] 100% 동일한 크기, 백색 숫자, 테두리 없이 테마 컬러가 꽉 채워지는 그리드 박스 */
    .uniform-grid-7 {{
        display: grid;
...
"""
    ...
    if is_insta:
        ...
        for val in range(6, 13):
            ...
            html_grid += f'<a href="?set_insta={val}" target="_self" class="box-item{active_cls}">{val}</a>'
        html_grid += '</div>'
        ...
        <div style="font-size:14px; font-weight:700; color:#e3e3e3; margin-bottom:8px; display:flex; align-items:center; gap:7px;">
            <span style="display:inline-block; width:8px; height:8px; border-radius:50%; background:{theme_bg};"></span>
            블로그 사진 장수 (15~20장) - 선택됨: <b style="color:{active_color}; font-size:15px;">{st.session_state.blog_photo_count}장</b>
        </div>
        """, unsafe_allow_html=True)
        
        # 15부터 20까지 총 6개 박스를 6열 그리드로 배치하여 완벽 동일 크기 유지
        ...
        for val in photo_vals:
            ...
            html_grid += f'<a href="?set_blog={val}" target="_self" class="box-item{active_cls}">{val}</a>'
        html_grid += '</div>'
        st.markdown(html_grid, unsafe_allow_html=True)
...
1. [자막 스타일 규칙 (STRICT - 서술형 종결어미 전면 금지)]:
- '자막：' 영역에는 '~하는 느낌이에요', '~좋아요', '~추천해요', '~발라보세요' 등 문장형/서술형 어미를 절대 쓰지 마세요.
- 오직 숏폼 영상 자막 표준에 맞춰 **명사형/키워드형 단문(예: 'OO하는 느낌', '촉촉 탱탱한 결', '초밀착 수분 충전', '끈적임 없는 흡수')**으로만 간결하게 작성하세요.
- 각 장면의 자막은 [앞쪽 나레이션 명사형 요약 / 뒤쪽 나레이션 명사형 요약] 구조로 작성하며, 슬래시(/)는 단독 줄로 배치합니다.

2. [정확히 {target_scenes}개 핵심 씬 구성 (STRICT)]:
...
5. [나레이션 종결 어미 스타일 엄수]:
- 나레이션 대사에서 '~했다', '~해봤다', '~붙여봤다' 등 딱딱하게 끝나는 종결형 어미를 절대 쓰지 마세요.
- 부드러운 연결형이나 솔직한 구어체로 표현하세요 (~해보고, ~발라봤는데, ~직접 써보니까 등).

6. [썸네일 필수 구성 (베스트 1종 + 추천 5선)]:
...
자막：
(앞쪽 나레이션 명사형 요약 - 서술형 어미 금지)
/
(뒤쪽 나레이션 명사형 요약 - 서술형 어미 금지)
