...
- 각 장면의 자막은 [앞쪽 나레이션 명사형 요약 / 뒤쪽 나레이션 명사형 요약] 구조로 작성하며, 슬래시(/)는 절대 사용하지 마세요.
...
자막：
(앞쪽 나레이션 명사형 요약 - 느낌표 및 슬래시 금지)
(뒤쪽 나레이션 명사형 요약 - 느낌표 및 슬래시 금지)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿과 [카테고리: {current_cat}], [정확히 {target_scenes}개 장면 구성], [자막 슬래시 및 서술형 금지], [나레이션 특수문자 금지 및 공백 포함 300자 이내], [가로 스크롤 방지 20~30자 줄바꿈], [시술명 금지], [베스트 썸네일 + 추천 5선]을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
