...
- **[매우 중요]**: 나레이션에서 브랜드명과 제품명이 모두 언급될 때만 자막에 `[브랜드명](로고 삽입) [제품명] 사용` 형태로 표기합니다. 나레이션에 제품명만 언급되고 브랜드명이 언급되지 않았다면 `[브랜드명](로고 삽입)` 형태를 절대 사용하지 마세요.
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [장면 수: {target_scenes}개], [브랜드명: {brand_name}], [제품명: {product_name}], [나레이션에 브랜드+제품 동시 언급 시에만 자막에 [브랜드명](로고 삽입) 제품명 사용 표기], [나레이션 2줄 작성], [가로 스크롤 방지 줄바꿈], [기울임꼴 금지], [각주 A* (하단 각주 삽입) *각주내용 형식])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
