...
- 자막에는 절대 서술형(~해라, ~해보세요, ~합니다, ~해요 등)을 쓰지 마세요. 오직 명사형/요약형 단문 및 키워드만 허용합니다.
...
- 자막의 줄과 줄 사이에는 반드시 슬래시(`/`)를 단독 줄로 배치하세요.

5. [가로 스크롤 방지 및 강제 줄바꿈 (CRITICAL)]:
- 모든 텍스트는 절대 한 줄로 길게 쓰지 말고, 20~30자 내외마다 반드시 엔터(줄바꿈)를 쳐서 가로 스크롤이 절대 생기지 않게 하세요. (기울임꼴 금지)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
...
[매우 중요: 블로그 필수 해시태그 세로 줄바꿈 및 엄격한 규칙 (CRITICAL)]
1. [가이드 필수 내용 100% 반영]:
...
3. [가로 스크롤 방지 강제 줄바꿈]:
- 촬영 가이드, 본문 원고 등 모든 텍스트는 25~35자 내외마다 반드시 엔터(줄바꿈)를 쳐서 가로 스크롤이 절대 생기지 않게 하세요.

4. [필수 해시태그 세로 줄바꿈 출력 (절대 규칙)]:
- 최하단 [필수 해시태그] 영역에서는 지정된 해시태그들을 한 줄에 길게 쓰지 말고, **반드시 각 해시태그마다 엔터를 쳐서 세로로 한 줄씩 아래로 나열**하세요. (옆으로 나열하거나 콤마로 묶는 것 절대 금지)
...
#해시태그3
#해시태그4
(이와 같이 반드시 한 줄에 하나씩 세로로 출력할 것)
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 사진/문구/내용 100% 누락 없이 반영], [실제 브랜드명 {brand_name} 및 제품명 {product_name} 적용], [필수 해시태그 무조건 한 줄에 하나씩 세로 줄바꿈 나열], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [가로 스크롤 방지 강제 줄바꿈], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
