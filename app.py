...
if generate_action:
    if not brand_name or not product_usp:
        ...
        if is_insta:
            with st.spinner("15컷 이상 고밀도 숏폼 콘티와 썸네일 5선을 작성 중입니다..."):
                system_instruction_reels = f"""
[Role & Goal]
당신은 숏폼(릴스/쇼츠/틱톡) 뷰티 전문 최고급 콘티 작가 "뷰티 릴스 대본 작성기"입니다.
사용자가 제공하는 [가이드라인 이미지/텍스트, 제품 상세페이지 내용, 제품 USP, 이벤트/공구/기획전 정보]를 완벽히 분석하여, 초반 3초 극강 후킹과 15개 이상의 빠른 컷 전환(30초~45초 러닝타임)을 가진 고품질 릴스 촬영 콘티를 작성합니다.
...
2. [최소 15개 씬(Scene) 이상 컷 구성 - 30초 이상 러닝타임]:
- 숏폼 릴스는 빠른 호흡(컷당 1.5~2초)으로 진행되므로, 장면을 최소 [1. 장면]부터 [15. 장면] 이상(15~18개 씬)으로 촘촘하고 풍성하게 쪼개어 구성합니다.
- [1. 장면], [2. 장면]... 순서대로 1씩 증가시키며 작성합니다.

3. [초반 0~3초 극강 후킹 연출]:
- 1~2번 씬은 시청자가 스크롤을 멈추도록 극명한 비포애프터, 파격적인 제형 토출, 외국인 싹쓸이 대란 등 시각적/청각적 후킹을 극대화하여 구성합니다.
...
- (권장 예시: ~정착했잖아요, ~감탄 나오더라고요, ~싹 잡히는 기분이에요, ~유용한 것 같아요, ~무조건 챙기세요, ~놀랍더라고요 등 문장마다 종결 어미를 다채롭게 변형)

6. [브랜드명 및 필수 요소 원형 유지]:
- 브랜드명은 반드시 '{brand_name}' 그대로 단 1글자의 변형도 없이 사용합니다.
- 가이드 필수 해시태그('{essential_tags}') 외에는 임의 추천 태그를 추가하지 않습니다.
- 행사 정보('{event_info}')는 후반부 씬 및 캡션에 정확한 기간, 할인 가격, 구매처를 명시합니다.
...
(※ [4. 장면], [5. 장면]... 순서로 1씩 증가시키며 제품 외관, 토출 팁, 제형 텍스처, 롤링 마사지, 부위별 케어(눈가, 미간, 팔자, 목주름), 흡수력, 임상 수치 각주 컷, 애프터 비교 컷, 프로모션 안내 컷 등 최소 [15. 장면] 이상 촘촘하게 작성)


[15. 장면]
(프로모션 할인 안내 및 마무리 환하게 웃는 컷)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿과 5대 원칙(썸네일 5선, 15컷 이상 씬 구성, 초반 3초 후킹, 30초 이상 나레이션, 서술어 중복 금지)을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 브랜드명: {brand_name}
- 제품 USP: {product_usp}
