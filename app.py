...
1. [자막 스타일 및 각주 규칙 (STRICT)]:
- '자막：' 영역에는 문장형/서술형 어미를 절대 쓰지 말고 오직 명사형/키워드형 단문으로만 작성하세요. 슬래시(/)는 절대 사용하지 마세요.
- 가이드라인에 임상시험 수치나 인증 정보 등 각주가 있는 경우, 해당 자막 밑에 `*`와 함께 각주를 표시하세요 (예: `괄사 쿨링 롤링 케어* \n *1회 사용 3분 후 즉각적 눈가 붓기 14% 개선`).
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿과 [카테고리: {current_cat}], [정확히 {target_scenes}개 장면 구성], [자막 슬래시 및 서술형 금지], [가이드 각주 * 포함 적용], [나레이션 특수문자 금지 및 공백 포함 300자 이내], [가로 스크롤 방지 20~30자 줄바꿈], [시술명 금지], [베스트 썸네일 + 추천 5선]을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
