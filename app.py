...
[브랜드명 및 고유 명칭 왜곡 절대 금지 (STRICT)]:
- 브랜드명(한글/영문/표기법 일체)은 사용자가 제공한 원문 그대로 단 1글자의 변형, 축약, 오역, 번역, 띄어쓰기 변경 없이 100% 동일하게만 사용합니다.
- 제품명, 라인명, 규격/중량, 소재, 디자인 등 구체적 수치와 명칭 역시 원형 그대로 유지합니다.
- 자막에 브랜드 이름이 들어가는 경우 반드시 [브랜드명](로고 삽입) 형태로 표기합니다 (예: 엑시스와이(로고 삽입)).

[해시태그 임의 생성/추가 절대 금지 (STRICT)]:
- 가이드라인/초안에 지정된 '필수 해시태그' 외에는 단 1개의 연관/추천/일반 해시태그도 임의로 추가하지 않습니다.
- 제공된 필수 해시태그가 없을 경우, 해시태그 항목은 완전히 비워두거나 공란으로 처리합니다.

[모든 제품, 구성품 및 행사 정보 100% 반영]:
- 가이드/초안에 2개 이상의 제품이나 부속품이 포함되어 있다면, 절대 누락하거나 축소하지 않고 순서대로 전부 다룹니다.
- 기획전 일정, 당일 특가 날짜, 할인 가격, 구매처를 임의 생략 없이 스크립트와 캡션에 모두 명시합니다.
...
- 각 씬 묶음 사이에는 구분선과 줄바꿈 여백을 반드시 넣습니다.
- 하단 '장면 요약' 섹션 역시 1. 장면：, 2. 장면：과 같이 순차 번호를 붙여 전체 씬을 나열합니다.

[자막 표기 및 특수 규칙 엄수]:
- 자막에는 이모티콘, 아이콘, 느낌표(!) 등 특수문자를 절대 사용하지 않습니다.
- 자막 줄바꿈 표기 시 한 줄에 /를 쓰지 않고, 반드시 줄바꿈하여 단독 줄로 /를 배치합니다 (오직 자막 컷 구분에만 슬래시 사용, 각주 내부나 문장 중간 슬래시 절대 금지).
- 비포/애프터 장면이 포함된 씬에서는 카메라 앵글 설명에 반드시 "비포&애프터 비교 컷"을 포함하고, 자막 영역에 반드시 "비포 애프터" 문구를 추가합니다.
- 각주 필수 삽입: 나레이션이나 자막에 각주 표시(*)가 있을 경우, 절대로 슬래시 다음 문장 밑에 달지 말고, 각주가 필요한 해당 자막 문구 바로 하단에 (하단 각주 삽입) 표시와 함께 *각주 원문 내용을 세트로 명시합니다.
...
자막：
(이모티콘, 아이콘, 느낌표 사용 금지 / 브랜드명 [브랜드명](로고 삽입) 표기 / 비포/애프터 씬인 경우 '비포 애프터' 기재 / 각주 존재 시 해당 문구 바로 하단에 (하단 각주 삽입) 및 *내용 기재)
자막 첫 문장
/
자막 둘째 문장

나레이션：
(특수문자 없이 공백 포함 300자 이내의 솔직하고 빠른 30대 여성 숏폼 구어체 / 지정된 정확한 브랜드명 사용)
...
나레이션：
(특수문자 없이 공백 포함 300자 이내의 솔직하고 빠른 30대 여성 숏폼 구어체)
...
"""
                prompt_text = f"""
사용자가 제공한 원본 젬스 프롬프트의 모든 절대 원칙([브랜드명 왜곡 금지], [해시태그 임의 추가 금지], [슬래시 컷 구분], [각주 하단 세트 배치], [비포애프터 필수 표기], [나레이션 특수문자 금지 및 300자 이내])을 100% 준수하여 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
