...
1. [자막 내 브랜드명 표기 규칙 (STRICT)]:
- 자막에 브랜드 이름이 들어가는 경우 반드시 **[브랜드명](로고 삽입)** 형태로 표기하세요 (예: `엑시스와이(로고 삽입)`).

2. [슬래시(/) 사용 범위 엄격 제한 (STRICT)]:
- 슬래시(`/`)는 **오직 자막과 자막 사이의 컷 구분(줄바꿈)** 용도로만 사용하세요. 각주 내부나 문장 중간에는 절대 슬래시를 넣지 마세요.

3. [자막 및 각주 세트 배치 규칙 (STRICT)]:
- 임상시험 수치나 인증 정보 등 각주가 있는 경우, 각주가 필요한 해당 자막 문구 바로 아래에 세트로 묶어서 아래 포맷처럼 작성하세요.

  [필수 포맷 예시]:
  자막：
  올영 프로모션 할인 중 *
  (하단 각주 삽입)
  *화해 2026 상반기 효능 효과 안티에이징 아이케어 1위

4. [비포&애프터 씬 연출 및 자막 표기 규칙 (STRICT)]:
- 장면에 비포/애프터 변화 내용이 포함될 경우, 카메라 앵글 설명에 반드시 **"비포&애프터 비교 컷"**이라는 표현을 포함하세요.
- 해당 장면의 자막에는 반드시 **"비포 애프터"**를 명시하세요.

  [필수 씬 포맷 예시]:
  [3. 장면]
  눈가 붓기와 주름이 정돈되고 맑아진 비포&애프터 비교 컷 → 올리브영 할인 안내 화면과 환하게 웃는 인물
...
[장면 번호]
카메라 앵글 → 인물 행동 및 제품 포인트 흐름 (비포/애프터 씬인 경우 '비포&애프터 비교 컷' 필수 포함)
(20~30자 단위로 줄바꿈)

자막：
(브랜드명 [브랜드명](로고 삽입) 표기 / 비포/애프터 씬인 경우 '비포 애프터' 필수 포함 / 슬래시 컷 구분 / 각주는 해당 문구 바로 아래 세트 배치)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿과 [카테고리: {current_cat}], [정확히 {target_scenes}개 장면 구성], [브랜드명은 반드시 브랜드명(로고 삽입) 형태 표기], [비포/애프터 씬은 '비포&애프터 비교 컷' 및 '비포 애프터' 자막 필수 표기], [슬래시(/)는 오직 자막 컷 구분에만 사용], [각주는 해당 문구 바로 아래에 세트 배치], [나레이션 특수문자 금지 및 공백 포함 300자 이내], [가로 스크롤 방지 20~30자 줄바꿈], [시술명 금지], [베스트 썸네일 + 추천 5선]을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
