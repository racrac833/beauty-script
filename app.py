...
1. [자막 및 각주 분리 표기 원칙 (STRICT)]:
- 자막 내 컷 구분은 반드시 슬래시(/)를 사용하여 분할하세요.
- 가이드라인에 임상시험 수치나 인증 정보 등 각주가 있는 경우, 자막 한 줄에 섞어 쓰지 말고 반드시 아래 예시 포맷처럼 하단에 분리하여 명시하세요.
...
자막：
(슬래시 / 로 컷이 구분된 키워드. 각주가 필요한 경우에만 예시처럼 (하단 각주 삽입)과 * 내용 명시)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿과 [카테고리: {current_cat}], [정확히 {target_scenes}개 장면 구성], [자막 슬래시(/) 컷 구분 및 각주 하단 분리 표기], [나레이션 특수문자 금지 및 공백 포함 300자 이내], [가로 스크롤 방지 20~30자 줄바꿈], [시술명 금지], [베스트 썸네일 + 추천 5선]을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
