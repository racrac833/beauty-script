...
[매우 중요: 자막 서술형 절대 금지 및 엄격한 규칙 (CRITICAL)]
1. [자막 서술형 100% 차단 (절대 규칙)]:
- **자막에는 절대 서술형(~해라, ~해보세요, ~합니다, ~해요, ~했더니 등)을 쓰지 마세요.**
- 자막은 오직 나레이션의 핵심 내용을 압축한 **명사형/요약형 단문, 키워드 형태**로만 작성하세요. (예: "마스크팩 에센스 꿀팁", "에센스 밀착 보습", "모서리 컷팅 팁" 등)
- 서술형 문장이 자막에 포함되면 절대 안 됩니다.

2. [가이드 필수 내용 100% 반영]:
- 가이드라인에 명시된 필수 장면, 필수 자막 키워드, 필수 나레이션은 누락 없이 반영하세요.

3. [브랜드명 표기 규칙 (조건부 발화)]:
- 대괄호 플레이스홀더(`[브랜드명]`, `[제품명]`) 금지.
- **나레이션 대사 안에 실제 브랜드명({brand_name})이 명시적으로 직접 발화되어 언급될 때만** 자막에 `{brand_name}(로고 삽입) {product_name} [명사형 키워드]` 형태로 표기합니다. 나레이션에 브랜드명이 없으면 자막에도 절대 넣지 마세요.

4. [나레이션 분량 및 자막 슬래시 구조]:
- 전체 나레이션 총합 분량은 **공백 포함 280자 이내** 엄수.
- 자막의 줄과 줄 사이에는 반드시 **슬래시(`/`)를 단독 줄로 배치**하고, 나레이션 양에 따라 자막 묶음을 동적으로 늘려 배치하세요.

5. [가로 스크롤 방지 및 강제 줄바꿈 (CRITICAL)]:
- 모든 텍스트는 절대 한 줄로 길게 쓰지 말고, 20~30자 내외마다 **반드시 엔터(줄바꿈)**를 쳐서 브라우저 화면에서 가로 스크롤이 절대 생기지 않게 하세요. (기울임꼴 금지)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [자막 서술형 절대 금지 및 오직 명사형/요약형 키워드 단문만 허용], [나레이션 내 실제 브랜드명 {brand_name} 발화 시에만 브랜드+제품+키워드 자막 표기], [나레이션 공백포함 280자 이내], [자막 줄 사이에 / 슬래시 단독 줄 배치 및 동적 묶음 확장], [추천 썸네일 띄어쓰기 철저 준수], [출력물 내 프롬프트 지시어 삭제], [가로 스크롤 방지 강제 줄바꿈 필수], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
