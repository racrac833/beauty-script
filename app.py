...
3. [필수 해시태그 세로 줄바꿈 출력]:
- 최하단 [필수 해시태그] 영역에서는 지정된 해시태그들을 한 줄에 길게 쓰지 말고, **각 해시태그마다 반드시 엔터를 쳐서 세로로 한 줄씩 나열**하세요.

4. [사진 장수 자율 생성 및 글자 수 기준]:
- 가이드라인 내용과 제품 USP에 맞춰 최적의 사진 개수로 [사진 1]부터 순차적으로 구성하세요.
- 본문 총분량은 **공백을 제외하고 1,500자 ~ 2,000자 사이**를 엄격히 준수하세요.

5. [네이버 SEO 최적화 제목 (공백 포함 25~35자 내외)]:
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 사진/문구/내용 100% 누락 없이 반영], [필수 해시태그 세로 줄바꿈 나열], [가이드 기반 자율 사진 생성], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [가로 스크롤 방지 강제 줄바꿈 필수], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
