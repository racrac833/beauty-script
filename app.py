...
자막：
(브랜드명 표기 시 [브랜드명](로고 삽입) 준수 / 자막 컷 구분에만 슬래시 사용 / 각주는 해당 문구 바로 아래 세트 배치 / 각주 내 슬래시 금지)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿과 [카테고리: {current_cat}], [정확히 {target_scenes}개 장면 구성], [브랜드명은 반드시 브랜드명(로고 삽입) 형태 표기], [슬래시(/)는 오직 자막 컷 구분에만 사용], [각주는 해당 문구 바로 아래에 세트 배치 및 각주 내 슬래시 금지], [나레이션 특수문자 금지 및 공백 포함 300자 이내], [가로 스크롤 방지 20~30자 줄바꿈], [시술명 금지], [베스트 썸네일 + 추천 5선]을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
