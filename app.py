...
def validate_and_fix_result(text, brand, product):
    ...
    3. 본문 텍스트, 캡션, 번호 매기기 항목 등에서 공백 제외 글자수가 30자가 넘어가면 
    ...
    for line in lines:
        ...
        # ⭐️ 핵심 보완: 공백을 제외한 글자 수가 30자 이상이면 파이썬이 강제로 쪼개서 줄바꿈 삽입
        no_space_len = len(stripped.replace(" ", ""))
        if no_space_len > 30 and not stripped.startswith('http') and not stripped.startswith('-'):
            # 마침표, 물음표, 느낌표 또는 조사/어미 기준으로 강제 분할
...
4. [본문 원고 30자 이상 시 강제 줄바꿈 & 해시태그 세로 나열 & 별표(**) 금지]:
...
[본문 원고]
(이 사진에 해당하는 상세 찐후기 원고 / 공백 제외 30자 이상 시 반드시 엔터 줄바꿈)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [기계식 인사말 절대 금지 및 자연스러운 후기 톤], [블로그 추천 제목 5선 포함], [총 15장~20장의 풍성한 사진 컷 구성], [사진 설명 바로 밑에 해당 본문 원고 1:1 밀착 매칭], [본문 원고는 공백 제외 30자 이상 시 무조건 엔터 쳐서 줄바꿈], [마크다운 별표(**) 사용 금지], [해시태그 무조건 세로로 한 줄씩 나열], [가이드 필수 내용 100% 반영], [공백 제외 글자수 1500~2000자 내외], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
