...
[핵심 절대 원칙 (STRICT CRITICAL - 줄바꿈 강제)]
1. [가로 스크롤 방지 및 줄바꿈 절대 준수]:
- 모든 텍스트(썸네일 설명, 카메라 앵글/행동 설명, 자막, 나레이션, 하단 주의점, 광고 캡션, 요약 섹션 등)는 **절대로 한 줄로 길게 길게 작성하는 것을 금지**합니다.
- 화면에서 가로 스크롤이나 긴 줄바꿈 압박이 생기지 않도록, **모든 문장과 설명은 20~30자 내외마다 반드시 엔터(줄바꿈)**를 쳐서 세로로 길게 읽히도록 작성하세요.

2. [장면 수 자율 생성 및 나레이션 분량]:
- 가이드라인과 제품 USP에 맞춰 최적의 장면 수로 구성하세요.
- 전체 나레이션 총합 분량은 **공백 포함 280자 ~ 300자 내외**를 엄격히 준수하세요.

3. [기울임꼴(Italic) 절대 금지]:
- 마크다운 이탤릭 기호(* 또는 _)를 절대 사용하지 마세요. 모든 글씨체는 기본 정체로만 출력합니다.

4. [인스타그램 자막 및 브랜드명 표기 규칙]:
...
### [1. 장면]
카메라 앵글 및 행동 연출 흐름 설명 / 
(반드시 20~30자마다 엔터 쳐서 줄바꿈)
...
(가이드라인에 맞춘 최적의 씬 개수만큼 '### [2. 장면]' 형식으로 순차적 반복하되, 모든 텍스트 줄바꿈 필수)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 기반 자율 장면 생성], [나레이션 총합 공백포함 280~300자], [브랜드명: {brand_name}], [제품명: {product_name}], [모든 텍스트 가로 스크롤 방지 강제 줄바꿈 필수], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
...
[네이버 블로그 SEO 및 줄바꿈 강제 원칙 (STRICT CRITICAL)]
1. [한 줄 쓰기 절대 금지 및 강제 줄바꿈]:
- 촬영 가이드 설명, 본문 원고 텍스트, 제목, 해시태그 등 모든 텍스트 영역에서 **절대로 한 줄로 길게 작성하지 마세요.**
- 모든 문장은 25~35자 내외마다 **반드시 엔터(줄바꿈)**를 쳐서 브라우저 화면에서 가로 스크롤이 절대 생기지 않도록 세로로 풍성하게 작성하세요.

2. [사진 장수 자율 생성 및 글자 수 기준]:
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [가이드 기반 자율 사진 생성], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [모든 텍스트 가로 스크롤 방지 강제 줄바꿈 필수], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
