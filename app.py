...
[매우 중요: 자막 제품명 온전한 유지, 강제 줄바꿈, 자막 포맷, 비포애프터 규칙 (ABSOLUTE CRITICAL)]
1. [브랜드명 언급 시 자막 제품명 결합 규칙 (핵심)]:
- 나레이션에 브랜드명({brand_name})이 언급될 때, 자막 첫 줄에는 반드시 **`{brand_name} (로고 삽입) {product_name}`**이 쪼개지지 않고 **하나의 온전한 덩어리**로 세팅되어야 합니다. (예: `VT코스메틱 (`) 로 쪼개거나 제품명을 분리하는 행위 절대 금지.
- 자막은 나레이션 내용을 요약한 명사형 단문이어야 합니다.

2. [30자 초과 시 강제 줄바꿈 (절대 규칙)]:
- 썸네일 설명, 씬별 촬영 컷 설명, 자막, 나레이션, 하단 구성 요소, 광고 캡션 등 모든 텍스트 영역에서 공백 제외 글자 수가 30자 이상이면 무조건 엔터(줄바꿈)`\n`을 쳐서 가로 스크롤을 완전히 차단하세요.

3. [자막 서술형 절대 금지 및 슬래시 포맷 (핵심 규칙)]:
- 자막에는 절대 서술형(~해라, ~해보세요 등)을 쓰지 말고, 오직 명사형 단문만 사용하세요.
- 자막이 두 줄 이상인 경우, 반드시 아래처럼 **한 줄 띄우고 슬래시(`/`)를 단독 줄로 배치한 뒤 다시 한 줄을 띄어** 작성하세요:
  [자막 첫 줄 내용 (브랜드명+제품명 또는 명사형)]
  
  /
  
  [자막 둘째 줄 내용 (명사형 행동/효과)]

4. [비포애프터(B/A) 자막 강제 삽입 규칙]:
- 장면 설명에 비포, 애프터, B/A, 비교 등의 키워드가 포함되어 있다면, 해당 장면의 자막 영역에 반드시 독립된 줄로 `(비포애프터)`라는 한글 표기를 포함하세요.

5. [가이드 필수 내용 100% 반영 & 분량]:
- 가이드라인 필수 항목을 100% 반영하고, 전체 나레이션 총합 분량은 공백 포함 280자 이내 엄수. 기울임꼴 금지.

[출력 양식 템플릿]
썸네일
썸네일(비주얼 연출 컷 설명) : 
(핵심 후킹 비주얼 연출 묘사 / 
공백 제외 30자 이상 시 무조건 줄바꿈)
...
자막：
(절대 서술형 금지 / 명사형 단문 / 
브랜드명 언급 시 {brand_name} (로고 삽입) {product_name} 온전한 결합 / 
비포애프터 포함 시 (비포애프터) 별도 줄 추가 / 
반드시 한 줄 띄우고 / 슬래시 단독 줄 배치 후 한 줄 띄우기)
자막 첫 번째 줄 (명사형)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [나레이션에 브랜드명이 언급될 때 자막에 {brand_name} (로고 삽입) {product_name} 형태로 절대 쪼개지 않고 온전한 덩어리로 표기], [자막은 나레이션을 요약한 명사형 단문], [모든 텍스트 영역에서 공백 제외 30자 이상 시 무조건 강제 줄바꿈], [자막 서술형 절대 금지 및 명사형 단문], [장면 설명에 비포/애프터/B/A 포함 시 자막에 반드시 (비포애프터) 단독 줄 추가], [자막 포맷: 한 줄 띄우고 / 슬래시 단독 줄 배치 후 한 줄 띄우기], [나레이션 공백포함 280자 이내], [출력물 내 프롬프트 지시어 삭제], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
...
[매우 중요: 공백 제외 30자 이상 시 무조건 강제 줄바꿈 및 해시태그 세로 나열 (ABSOLUTE CRITICAL)]
1. [30자 초과 시 강제 줄바꿈 (절대 규칙)]:
- 제목, 촬영 가이드, 본문 원고 텍스트 등 블로그 원고의 모든 텍스트 영역에서 **공백을 제외한 글자 수가 30자 이상이 되면 무조건 엔터(줄바꿈)`\n`을 쳐서** 브라우저 화면에서 가로 스크롤이 절대 생기지 않게 하세요.

2. [필수 해시태그 무조건 세로로 한 줄씩 나열]:
- 최하단 [필수 해시태그] 영역에서는 지정된 해시태그들을 한 줄에 길게 쓰지 말고, **반드시 각 해시태그마다 엔터를 쳐서 세로로 한 줄씩 아래로 나열**하세요.

3. [가이드 필수 내용 100% 반영 & 분량 기준]:
- 가이드라인 필수 내용을 원고 구석구석에 녹여내어 반영하세요.
- 본문 총분량은 공백 제외 1,500자 ~ 2,000자 사이 엄수. 제목은 공백 포함 25~35자 내외 5선 추천. '시술' 등 의료 표현 금지. ~해봤는데요 톤 유지.

[출력 양식 템플릿]
[네이버 블로그 추천 제목 5선 (SEO 최적 글자수 25~35자)]
1. (브랜드명 및 키워드 배치 / 공백 제외 30자 이상 시 줄바꿈)
2. 
3. 
4. 
5. 
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 사진/문구/내용 100% 누락 없이 반영], [모든 텍스트 영역에서 공백 제외 30자 이상 시 무조건 강제 줄바꿈], [필수 해시태그 무조건 한 줄에 하나씩 세로 줄바꿈 나열], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
