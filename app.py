...
def validate_and_fix_result(text, brand, product):
    """
    파이썬 후처리 검증 엔진 (안전 보호 모드):
    1. 마크다운 볼드체(**) 제거
    2. [베스트 썸네일], [추천 썸네일 문구 9선] 영역은 띄어쓰기가 절대 훼손되지 않도록 보호
    3. 오직 광고 캡션 영역에서만 문장 단위 줄바꿈 및 해시태그 세로 정렬 적용
    """
    if not text:
        return text
    ...
    for line in lines:
        ...
        # 썸네일 관련 섹션은 후처리의 띄어쓰기 뭉개기 대상에서 제외하여 원본 보호
        if "[베스트 썸네일]" in line or "[추천 썸네일 문구 9선]" in line:
            in_thumbnail_section = True
            processed_lines.append(line)
            continue
        elif "메인 스크립트" in line or "### [" in line:
            in_thumbnail_section = False

        if in_thumbnail_section:
            # 썸네일 영역은 띄어쓰기를 건드리지 않고 그대로 유지
            processed_lines.append(line)
            continue
...
- **썸네일 문구 작성 시 단어 사이의 띄어쓰기를 반드시 포함하여 자연스럽게 작성하세요.** (예: `마스크팩 이렇게 쓰면 피부 우유광 터집니다`)
...
- [나레이션만 정리] 영역은 각 나레이션을 문장마다 엔터를 쳐서 세로로 분할하세요.
