...
if generate_action:
    if not brand_name or not product_usp:
        ...
        if is_insta:
            with st.spinner("썸네일 5선 및 7~8개 핵심 씬 구성의 30초 이상 고품질 릴스 콘티를 작성 중입니다..."):
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([SEO 최적 25~35자 제목 5선], [사진 15장 풀세트 가이드], [가독성 줄바꿈], [~했다 어미 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 브랜드명: {brand_name}
- 제품 USP: {product_usp}
- 행사/가격 정보: {event_info if event_info else '가이드 참조'}
- 타겟층: {target_audience}
