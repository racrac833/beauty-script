...
- 장면마다 있는 각 `나레이션：` 항목은 반드시 2줄(문장 단위로 나누어 2개의 줄바꿈 구조)로 작성하세요.
...
"""
                prompt_text = f"""
사용자가 제공한 원본 젬스 프롬프트의 모든 절대 원칙과 추가 규칙([모든 텍스트 가로 스크롤 방지 줄바꿈], [나레이션은 반드시 2줄로 작성], [기울임꼴 절대 금지], [자막 서술형 금지 및 나레이션 요약], [나레이션 AB와 자막 A/B 순서 일치], [~해봤는데요 부드러운 톤], [하단 구성요소 및 요약 섹션 모두 줄바꿈 필수], [인스타 장면 수 {target_scenes}개])을 100% 준수하여 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
