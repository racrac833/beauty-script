...
[매우 중요: 가이드 필수 항목 및 엄격한 표기 규칙 (CRITICAL)]
1. [가이드 필수 내용 100% 반영]:
- 제공된 가이드라인에 명시된 필수 장면, 필수 자막, 필수 나레이션, 필수 안내 사항은 절대 누락하지 말고 100% 반영하세요.

2. [브랜드명 및 제품명 실제 값 적용 (매우 중요)]:
- 대본 작성 시 `[브랜드명]`이나 `[제품명]` 같은 대괄호 형태의 플레이스홀더를 절대 쓰지 마세요.
- 반드시 입력된 **실제 브랜드명({brand_name})**과 **실제 제품명({product_name})**을 정확히 기재하여 작성하세요.
- 브랜드명과 제품명이 모두 언급될 때만 자막에 `{brand_name}(로고 삽입) {product_name} 사용` 형태로 표기합니다.

3. [비포&애프터 자막 규칙]:
- 장면 연출 설명이나 대본 내용 중 피부 변화나 비포/애프터 비교를 다루는 장면이 있다면, 해당 씬의 자막에 반드시 `비포 애프터` 문구를 포함하세요.

4. [추천 썸네일 문구 띄어쓰기 철저 준수]:
- 추천 썸네일 문구 9선의 각 문장(첫째 줄, 둘째 줄)은 **단어 간 띄어쓰기를 반드시 포함**하여 자연스러운 한국어로 작성하세요. (글자 수 제한: 첫째 줄 공백 제외 10자 이내, 둘째 줄 공백 제외 12자 이내)

5. [가로 스크롤 방지 및 강제 줄바꿈 (출력물에 프롬프트 지시어 노출 금지)]:
- 대본 결과물 내에 `(줄바꿈 필수)` 같은 시스템 지시어 문구를 절대 출력하지 마세요.
- 모든 텍스트는 20~30자 내외마다 엔터(줄바꿈)를 쳐서 세로로 읽히도록 작성하세요.

6. [나레이션 분량 및 기타 원칙]:
- 전체 나레이션 총합 분량은 공백 포함 280자 ~ 300자 내외 엄수.
- 기울임꼴(* 또는 _) 절대 금지. 대사 내 느낌표(!), 슬래시(/) 등 특수문자 금지. ~해봤는데요 톤 유지. 지정된 필수 해시태그 외 임의 추가 금지.
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [실제 브랜드명 {brand_name} 및 제품명 {product_name} 적용 (대괄호 금지)], [비포 애프터 자막 적용], [추천 썸네일 띄어쓰기 철저 준수], [출력물 내 프롬프트 지시어 일체 삭제], [가로 스크롤 방지 강제 줄바꿈], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
...
2. [실제 브랜드명 및 제품명 적용 (매우 중요)]:
- 원고 내에 `[브랜드명]`이나 `[제품명]` 같은 대괄호 형태의 플레이스홀더를 절대 쓰지 마세요.
- 반드시 입력된 **실제 브랜드명({brand_name})**과 **실제 제품명({product_name})**을 정확히 기재하여 작성하세요.

3. [출력물 내 프롬프트 지시어 노출 금지 및 강제 줄바꿈]:
- 원고 결과물 내에 `(줄바꿈 필수)` 같은 시스템 지시어 문구를 절대 출력하지 마세요.
