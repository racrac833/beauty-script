...
- 각주가 있는 경우, A* (하단 각주 삽입) *각주내용 형식으로, ABC 중에서 A가 각주 해당 단어라면 A에 * 표시하고, A*에 대한 각주내용은 또다시 *를 맨 앞에 쓰고 각주내용을 작성하세요 (예: 900샷 마이크로니들 구조 * \n (하단 각주 삽입) \n *유효 성분 흡수에 한함). 절대 슬래시 다음 문장 밑에 각주를 달지 마세요.
...
나레이션：
(딱딱한 ~해봤습니다 금지 / ~해봤는데요 등 30대 여성 부드러운 찐후기 구어체 / 공백 포함 300자 이내 / 특수문자 금지 / 반드시 2줄로 나누어 작성 / 가로 스크롤 방지 줄바꿈 적용)
첫 번째 나레이션 문장
두 번째 나레이션 문장
...
나레이션：
(~해봤는데요 톤 / 반드시 2줄로 나누어 작성 / 가로 스크롤 방지 줄바꿈 적용)
첫 번째 나레이션 문장
두 번째 나레이션 문장
...
썸네일 재요약
썸네일(연출 설명) : (상단 썸네일 비주얼 연출 설명 재출력 / 가로 스크롤 방지 줄바꿈 적용)
...
"""
                prompt_text = f"""
사용자가 제공한 원본 젬스 프롬프트의 모든 절대 원칙과 추가 규칙([모든 텍스트 가로 스크롤 방지 줄바꿈], [나레이션은 반드시 2줄로 작성], [기울임꼴 절대 금지], [자막 서술형 금지 및 나레이션 요약], [나레이션 AB와 자막 A/B 순서 일치], [~해봤는데요 부드러운 톤], [각주는 A* (하단 각주 삽입) *각주내용 형식 엄수], [하단 구성요소 및 요약 섹션 모두 줄바꿈 필수], [인스타 장면 수 {target_scenes}개])을 100% 준수하여 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
...
- 선케er/클렌징: 백탁/눈시림 여부, 세정력 테스트, 잔여감 없는 산뜻함 중심
