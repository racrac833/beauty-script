...
def validate_and_fix_result(text, brand, product):
    ...
    파이썬 후처리 검증 엔진 (강화 버전):
    1. 마크다운 볼드체(**) 전면 제거
    2. 긴 한 줄 문장(번호 항목 포함)을 조사/접속사/구두점 기준으로 강제 줄바꿈
    3. 해시태그 세로 정렬
    """
    if not text:
        return text
    ...
    for line in lines:
        ...
        # 30자가 넘는 긴 문장이나 번호 매기기 문장이 한 줄로 길게 있으면 강제 분할
        if (in_caption or re.match(r'^\d+\.', stripped)) and len(stripped) > 30:
            # 마침표, 물음표, 느낌표 뿐만 아니라 주요 접속사/조사(~서, ~며, ~고, ~서, ~데) 기준으로도 쪼갬
            sub_sentences = re.split(r'(?<=[.?!])\s+|(?<=[고서며데네죠])\s+', stripped)
...
- 두 줄 이상 자막은 반드시 아래 포맷처럼 **한 줄 띄우고, 슬래시(`/`)를 단독 줄로 배치한 뒤, 다시 한 줄을 띄어** 작성하세요:
  [자막 첫 줄 (브랜드명+제품명 또는 명사형)]
  
  /
  
  [자막 둘째 줄 (명사형 행동/효과)]

6. [비포애프터(B/A) 자막 강제 삽입]:
- 장면 설명에 비포, 애프터, B/A, 비교 등의 키워드가 포함되어 있다면, 자막 영역에 반드시 독립된 줄로 `(비포애프터)`를 추가하세요.

7. [마크다운 강조(**) 금지 및 줄바꿈 규칙]:
- **결과물 내에 마크다운 볼드체 별표(**)를 절대 사용하지 마세요.**
- [나레이션만 정리] 영역은 각 나레이션을 문장마다 반드시 엔터를 쳐서 세로로 분할하세요.
- [#광고 캡션] 영역은 문장마다 반드시 엔터를 쳐서 여러 줄로 나누어 작성하세요.
- [해시태그] 영역은 각 해시태그마다 반드시 엔터를 쳐서 세로로 한 줄씩 아래로 나열하세요.
...
#광고 캡션
(30대 여성 찐후기 톤 / 문장마다 반드시 엔터를 쳐서 여러 줄로 분할)
...
1. 첫 번째 나레이션 문장
2. 두 번째 나레이션 문장
(반드시 각 나레이션 문장마다 엔터를 쳐서 세로로 분할 정리)
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [썸네일은 첫줄 공백 제외 10자 이내, 둘째줄 공백 제외 12자 이내 엄수], [장면 설명은 의미 단위마다 엔터 쳐서 세로 분할], [메인 스크립트는 장면 ➔ 자막 ➔ 나레이션 순서 엄수], [나레이션에 브랜드명/제품명 언급 시 자막에 {brand_name} (로고 삽입) {product_name} 형태를 절대 쪼개지 말고 온전한 덩어리로 표기], [자막은 나레이션을 요약한 명사형 단문], [마크다운 별표(**) 사용 금지], [나레이션만 정리 영역은 각 나레이션을 반드시 엔터 쳐서 세로로 한 줄씩 분할], [캡션은 문장마다 엔터 쳐서 분할], [해시태그는 무조건 세로로 한 줄씩 나열], [자막 서술형 절대 금지 및 명사형 단문], [장면 설명에 비포/애프터/B/A 포함 시 자막에 반드시 (비포애프터) 단독 줄 추가], [자막 포맷: 한 줄 띄우고 / 슬래시 단독 줄 배치 후 한 줄 띄우기], [나레이션 공백포함 280자 이내], [출력물 내 프롬프트 지시어 삭제], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
...
[매우 중요: 블로그 최적화 사진 15~20장 및 1:1 밀착 매칭 규칙 (ABSOLUTE CRITICAL)]
1. [기계식 인사말 절대 금지]:
...
3. [사진 장수 15장~20장 확보 & 사진 설명 바로 아래 본문 원고 1:1 매칭]:
- **네이버 SEO 최적화와 상위 노출을 위해 반드시 총 15장 ~ 20장 분량의 사진 컷을 구성하세요.**
- 사진을 몰아서 나중에 쓰지 말고, **[사진 1] (촬영 가이드) 바로 밑에 그 사진에 맞는 상세 본문 원고를 즉시 매칭하고, 이어서 [사진 2] ~ [사진 15~20]까지 모든 사진마다 가이드와 본문 원고를 1:1 쌍으로 밀착 배치**하세요.

4. [본문 원고 한 줄 쓰기(뭉치기) 금지 & 해시태그 세로 나열 & 별표(**) 금지]:
- 본문 원고의 모든 문장은 문장마다 반드시 엔터(줄바꿈)를 쳐서 세로로 풍성하게 작성하세요. (한 줄 쓰기 엄격 금지)
- **결과물 내에 마크다운 볼드체 별표(**)를 절대 사용하지 마세요.**
...
[사진 1]
(촬영 가이드: 제품 언박싱 또는 패키지 전면 컷)

[본문 원고]
(이 사진에 해당하는 상세 찐후기 원고 / 문장마다 엔터 쳐서 줄바꿈)

-------------------------------------------------------

[사진 2]
(촬영 가이드: 패키지 측면 / 제형 토출 컷)

[본문 원고]
(이 사진에 해당하는 상세 찐후기 원고 / 문장마다 엔터 쳐서 줄바꿈)

(동일한 구조로 [사진 3]부터 [사진 15~20]까지 가이드와 본문 원고를 1:1로 끝까지 작성)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [기계식 인사말 절대 금지 및 자연스러운 후기 톤], [블로그 추천 제목 5선 포함], [총 15장~20장의 풍성한 사진 컷 구성], [사진 설명 바로 밑에 해당 본문 원고 1:1 밀착 매칭], [본문 원고는 문장마다 엔터 쳐서 줄바꿈], [마크다운 별표(**) 사용 금지], [해시태그 무조건 세로로 한 줄씩 나열], [가이드 필수 내용 100% 반영], [공백 제외 글자수 1500~2000자 내외], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
