...
1. [자막 스타일 및 컷 구분 규칙 (STRICT)]:
- '자막：' 영역에는 서술형 어미를 절대 쓰지 말고 명사형/키워드형 단문으로 작성하세요.
- 각 자막의 컷 구분은 반드시 슬래시(`/`)를 사용하여 템포감 있게 분할하세요 (예: `외국인이 쓸어 담는\n/\n올영 1위 괄사 아이세럼`).
- 비포/애프터 변화가 드러나는 씬에는 반드시 자막에 `비포 애프터` 키워드를 포함하세요.
- 가이드라인에 임상시험 수치나 인증 정보 등 각주가 있는 경우, 해당 자막 밑에 `*` 기호와 함께 각주를 표시하세요 (예: `괄사 쿨링 롤링 케어*\n*1회 사용 3분 후 즉각적 눈가 붓기 14% 개선`).

2. [나레이션 엄격 규칙 (CRITICAL - 글자수 및 특수문자 금지)]:
- 나레이션 대사에는 느낌표(!), 슬래시(/) 등 어떠한 특수문자나 기호도 절대 포함하지 마세요.
- 전체 나레이션의 총 글자 수는 공백을 포함하여 **30대 여성 찐후기 톤으로 간결하게 300자 이내**로 작성하세요.

3. [정확히 {target_scenes}개 핵심 씬 구성 (STRICT)]:
...
자막：
(슬래시 / 로 컷이 구분된 명사형 자막 키워드 / 필요시 * 및 각주 삽입)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿과 [카테고리: {current_cat}], [정확히 {target_scenes}개 장면 구성], [자막 슬래시(/) 컷 구분 및 비포 애프터 표기], [가이드 각주 * 포함 적용], [나레이션 특수문자 금지 및 공백 포함 300자 이내], [가로 스크롤 방지 20~30자 줄바꿈], [시술명 금지], [베스트 썸네일 + 추천 5선]을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
