...
2. [실제 브랜드명 및 제품명 적용 / '사용' 단어 삭제 및 명사형 키워드 대체]:
- 대괄호 형태의 플레이스홀더를 절대 쓰지 말고, 입력된 **실제 브랜드명({brand_name})**과 **실제 제품명({product_name})**을 정확히 기재하세요.
- 자막에 브랜드명을 표기할 때 절대 서술형(~합니다 등)을 쓰지 말고, `{brand_name}(로고 삽입) {product_name} [제품 특성에 맞는 명사형 키워드]` 형태로 쓰되, **'사용'이라는 단어는 절대 쓰지 말고** 해당 장면의 문맥과 관련된 명사형 단어(예: 진정 케어, 수분 충전, 밀착 발림 등)로 대체하세요.
...
5. [자막 슬래시 구분선 필수 및 동적 확장]:
- 두 줄 이상으로 구성된 자막은 줄과 줄 사이에 **반드시 슬래시(`/`)를 넣어 명확히 구분**하세요.
- 나레이션의 호흡과 양이 길어질 경우, 자막을 1개만 쓰지 말고 **분량에 맞춰 자막 묶음을 동적으로 늘려서** 매칭하세요.

6. [추천 썸네일 문구 띄어쓰기 철저 준수 & 줄바꿈]:
- 추천 썸네일 문구 9선의 각 문장은 단어 간 띄어쓰기를 반드시 포함하고, 모든 텍스트는 20~30자 내외마다 엔터(줄바꿈)를 쳐서 가로 스크롤을 방지하세요. (기울임꼴 금지)
...
자막：
(비포 애프터 또는 분량에 맞춘 명사형 단문 자막 세트 / 줄 사이에 반드시 / 슬래시 포함 / '사용' 단어 금지)
자막 첫 번째 줄
/
자막 두 번째 줄
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [실제 브랜드명 {brand_name} 및 제품명 {product_name} 적용 및 '사용' 단어 삭제 후 명사형 키워드 대체], [비포 애프터 자막 적용], [나레이션 공백포함 280자 이내], [두 줄 이상 자막 사이 / 슬래시 필수], [나레이션 분량에 따른 자막 세트 동적 확장], [추천 썸네일 띄어쓰기 철저 준수], [출력물 내 프롬프트 지시어 삭제], [가로 스크롤 방지 강제 줄바꿈], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
