...
def validate_and_fix_result(text, brand, product):
    """
    파이썬 후처리 교정 함수: 해시태그 강제 분할 로직을 제거하고, 
    오직 공백 제외 30자 초과 시 줄바꿈 기준만 관리합니다.
    """
    if not text:
        return text
    return text
...
1. [자막 제품명 온전한 덩어리 유지 (절대 규칙)]:
- 나레이션에 브랜드명({brand_name})과 제품명({product_name})이 언급될 때, 자막 첫 줄에는 절대 단어가 쪼개지지 않고 온전한 덩어리인 **`{brand_name} (로고 삽입) {product_name}`** 형태로만 들어가야 합니다. (예: `VT코스메틱 (로고 삽입) VT 피디알엔 에센스 시트 마스크`)
...
4. [비포애프터(B/A) 자막 강제 삽입]:
- 장면 설명에 비포, 애프터, B/A, 비교 등의 키워드가 포함되어 있다면, 자막 영역에 반드시 독립된 줄로 `(비포애프터)`를 추가하세요.

5. [공백 제외 30자 초과 시 강제 줄바꿈]:
- 모든 텍스트 영역에서 공백을 제외한 글자 수가 30자 이상일 때만 엔터(줄바꿈)`\n`을 쳐서 가로 스크롤을 방지하세요. (해시태그 등은 30자 미만이면 자연스럽게 한 줄로 유지)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 항목 100% 반영], [나레이션에 브랜드명/제품명 언급 시 자막에 {brand_name} (로고 삽입) {product_name} 형태를 절대 쪼개지 말고 온전한 덩어리로 표기], [자막은 나레이션을 요약한 명사형 단문], [공백 제외 30자 초과 시에만 강제 줄바꿈], [해시태그는 30자 미만 시 한 줄 유지], [자막 서술형 절대 금지 및 명사형 단문], [장면 설명에 비포/애프터/B/A 포함 시 자막에 반드시 (비포애프터) 단독 줄 추가], [자막 포맷: 한 줄 띄우고 / 슬래시 단독 줄 배치 후 한 줄 띄우기], [나레이션 공백포함 280자 이내], [출력물 내 프롬프트 지시어 삭제], [기울임꼴 금지])을 100% 지켜 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
...
[매우 중요: 공백 제외 30자 초과 시 강제 줄바꿈 (ABSOLUTE CRITICAL)]
1. [30자 초과 시 줄바꿈]:
- 제목, 촬영 가이드, 본문 원고 텍스트 등 블로그 원고의 모든 텍스트 영역에서 **공백을 제외한 글자 수가 30자 이상이 될 때만 엔터(줄바꿈)`\n`을 쳐서** 브라우저 화면에서 가로 스크롤이 절대 생기지 않게 하세요. (30자 미만인 해시태그 등은 한 줄 유지)
...
[원고 텍스트]
(가이드 필수 내용 및 30대 찐후기 반영 / 공백 제외 1500~2000자 목표 분량 준수)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 사진/문구/내용 100% 누락 없이 반영], [공백 제외 30자 초과 시에만 강제 줄바꿈], [해시태그 30자 미만 시 한 줄 유지], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
