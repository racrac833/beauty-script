...
<style>
    ...
    /* 8. 결과창 헤더 뱃지 및 기울임꼴 절대 금지 (기본 폰트 렌더링 정상화) */
    em, i {{
...
- 모든 텍스트(썸네일 설명, 씬별 연출 컷 설명, 나레이션 문장, 광고 캡션 등)는 한 줄 쓰기를 절대 금지하며, 워드 기본 폰트 기준 화면에서 가로 스크롤이 생기지 않도록 적당한 길이에서 엔터(줄바꿈)를 쳐서 작성하세요.
...
- 비포/애프터 씬인 경우 카메라 앵글에 "비포&애프터 비교 컷"을 기재하고, 자막에 "비포 애프터"를 표기하세요.
...
- 하단 장면 요약 역시 1. 장면：, 2. 장면：과 같이 순차 번호를 붙여 전체 씬을 나열합니다.
...
"""
                prompt_text = f"""
사용자가 제공한 원본 젬스 프롬프트의 모든 절대 원칙과 추가 규칙([줄바꿈 적용으로 스크롤 방지], [기울임꼴 절대 금지], [자막 서술형 금지 및 나레이션 요약], [나레이션 AB와 자막 A/B 순서 일치], [~해봤는데요 부드러운 톤], [인스타 장면 수 {target_scenes}개])을 100% 준수하여 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
