...
<style>
    ...
    /* 8. 결과창 헤더 뱃지 및 기울임꼴 절대 금지 (기본 폰트 정체 렌더링 강제) */
    em, i, * {{
...
- 모든 텍스트(썸네일 설명, 씬별 연출 컷 설명, 나레이션 문장, 광고 캡션, 썸네일 재요약, 나레이션만 정리를 포함한 모든 하단 요약 섹션)는 한 줄 쓰기를 절대 금지하며, 워드 기본 폰트 기준 화면에서 가로 스크롤이 생기지 않도록 적당한 길이에서 엔터(줄바꿈)를 쳐서 작성하세요.

2. [기울임꼴(Italic) 절대 금지 (STRICT)]:
- 모든 출력 텍스트에 마크다운 이탤릭 기호(* 또는 _)를 사용하여 글씨를 기울이지 마세요. 모든 글씨체는 기본 정체(Normal)로만 출력합니다. ((하단 각주 삽입) 등 안내 문구도 절대 기울이지 말고 일반 정체로 출력)

3. [인스타그램 자막 작성 규칙 (STRICT)]:
...
- 자막에 브랜드 이름이 들어가는 경우 반드시 [브랜드명](로고 삽입) 형태로 표기하세요.
- 비포/애프터 장면이 포함된 씬에서는 카메라 앵글에 "비포&애프터 비교 컷"을 기재하고, 자막에 "비포 애프터"를 표기하세요.
- 각주가 있는 경우, 슬래시 다음 문장 밑이 아니라 해당 자막 문구 바로 아래에 (하단 각주 삽입)과 *원문 내용을 세트로 배치하세요.

4. [나레이션 톤앤매너 및 구조 규칙 (STRICT)]:
- 나레이션은 작위적인 광고 문구를 지양하고, 30대 여성이 실제로 사용해보고 솔직하게 공유하는 자연스럽고 친근한 리얼 후기 어투로 작성합니다.
- "~해봤습니다", "~해봤다" 같이 딱딱한 표현은 절대 금지하며, 반드시 **"~해봤는데요"**처럼 30대 여성이 말하듯 부드럽고 자연스러운 표현만 사용합니다.
- 나레이션 대사에는 느낌표(!), 슬래시(/) 등 어떠한 특수문자나 기호도 절대 포함하지 않습니다.
- 전체 나레이션 총합 분량은 공백 포함 280자 ~ 300자 내외로 타이트하게 작성합니다.

5. [브랜드명 및 고유 명칭 왜곡 절대 금지 (STRICT)]:
- 브랜드명(한글/영문/표기법 일체)과 제품명, 라인명, 수치, 소재 등은 원형 그대로 100% 동일하게 유지합니다.

6. [해시태그 임의 생성/추가 절대 금지 (STRICT)]:
- 가이드라인/초안에 지정된 '필수 해시태그' 외에는 단 1개의 연관/추천/일반 해시태그도 임의로 추가하지 않습니다.

7. [장면 넘버링 및 가독성 분리]:
- 메인 스크립트의 각 씬 헤더는 반드시 ### [1. 장면], ### [2. 장면] 형태로 1씩 증가시키며 작성하고, 각 씬 사이에는 구분선(-------------------------------------------------------)을 넣습니다.
- 하단 '썸네일 재요약', '장면 요약', '나레이션만 정리' 등 모든 하단 섹션의 내용도 절대 한 줄 쓰기를 하지 말고, 항목별로 줄바꿈하여 가독성 있게 작성합니다.

[출력 양식 및 포맷 가이드 (Strict Rule)]
반드시 아래의 양식 구조와 섹션 구성을 한 줄도 빠짐없이 엄격히 지켜 답변을 생성하세요.

썸네일
썸네일(비주얼 연출 컷 설명) : (영상의 핵심 후킹을 담은 직관적인 비주얼 연출 묘사 / 가로 스크롤 방지 줄바꿈 적용)
...
자막：
(서술형 금지 / 나레이션 요약형 단문 / 나레이션 AB 순서와 A / B 자막 순서 완벽 일치 / 슬래시는 단독 줄 배치)
자막 요약 첫 문장
/
자막 요약 둘째 문장

나레이션：
(딱딱한 ~해봤습니다 금지 / ~해봤는데요 등 30대 여성 부드러운 찐후기 구어체 / 공백 포함 300자 이내 / 특수문자 금지 / 가로 스크롤 방지 줄바꿈 적용)
...
나레이션：
(~해봤는데요 톤 / 가로 스크롤 방지 줄바꿈 적용)

-------------------------------------------------------

(※ 선택된 장면 수 `{target_scenes}개`에 맞춰 '### [3. 장면]', '### [4. 장면]'... 순서대로 1씩 증가하며 마지막 씬까지 반복 작성)
...
#광고 캡션
(30대 여성 찐후기 톤: 리얼 후킹 문구 + 솔직 사용 경험 및 핵심 장점 + 기획전/특가 일정, 할인 가격, 구매처 안내 포함 / 가로 스크롤 방지 줄바꿈 적용)
...
썸네일 재요약
썸네일(연출 설명) : (상단 썸네일 비주얼 연출 설명 재출력 / 줄바꿈 적용)
(베스트 썸네일 첫줄 카피 - 띄어쓰기 필수, 공백 제외 10자 이내)
(베스트 썸네일 둘째줄 카피 - 띄어쓰기 필수, 공백 제외 12자 이내)

장면 요약

1. 장면： (1번 씬의 연출 요약 / 가로 스크롤 방지 줄바꿈 적용)
2. 장면： (2번 씬의 연출 요약 / 가로 스크롤 방지 줄바꿈 적용)
(※ 전체 씬의 연출 흐름을 번호 순서대로 누락 없이 한 줄씩 줄바꿈하여 나열)

나레이션만 정리
(전체 씬의 나레이션을 순서대로 모아서 출력 / 전체 글자 수: 공백 포함 280자 ~ 300자 엄수 / 문장 단위로 엔터를 쳐서 한 줄로 길게 출력되지 않도록 줄바꿈 적용)
"""
                prompt_text = f"""
사용자가 제공한 원본 젬스 프롬프트의 모든 절대 원칙과 추가 규칙([모든 텍스트 가로 스크롤 방지 줄바꿈], [기울임꼴 절대 금지], [자막 서술형 금지 및 나레이션 요약], [나레이션 AB와 자막 A/B 순서 일치], [~해봤는데요 부드러운 톤], [썸네일 재요약 및 나레이션만 정리 섹션 줄바꿈 필수], [인스타 장면 수 {target_scenes}개])을 100% 준수하여 인스타그램 숏폼 대본을 작성해줘:
- 카테고리: {current_cat}
...
2. [기울임꼴(Italic) 절대 금지 (STRICT)]:
- 모든 출력 텍스트에 마크다운 이탤릭 기호(* 또는 _)를 사용하여 글씨를 기울이지 마세요.
...
5. [의료/피부과 시술명 및 시술 비교 표현 절대 금지 (STRICT BAN)]:
