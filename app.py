...
1. [썸네일 설명 및 장면에 대한 세로 분할 줄바꿈 규칙 (절대 규칙)]:
- **[썸네일(비주얼 연출 컷 설명)]과 [### [X. 장면]] 아래의 촬영 컷 연출 설명은 절대 한 줄로 길게 쓰지 마세요.**
- 카메라 앵글, 모델의 행동, 제품 제형 연출 등 의미 단위마다 반드시 엔터(줄바꿈)를 쳐서 2~3줄 이상 세로로 풍성하게 나누어 작성하세요.
...
6. [나레이션만 정리, 캡션, 해시태그 줄바꿈 규칙 (매우 중요)]:
- **[나레이션만 정리] 영역은 모든 나레이션을 통째로 뭉쳐 쓰지 말고, 각 씬별 나레이션을 반드시 엔터(줄바꿈)를 쳐서 세로로 한 줄씩 독립되게 나열하세요.**
- **[#광고 캡션] 영역은 문장마다 반드시 엔터를 쳐서 여러 줄로 나누어 작성하세요.** (한 문단 뭉침 절대 금지)
- **[해시태그] 영역은 각 해시태그마다 반드시 엔터를 쳐서 세로로 한 줄씩 아래로 나열**하세요.
...
[매우 중요: 문장 단위 필수 줄바꿈 및 해시태그 세로 나열 (ABSOLUTE CRITICAL)]
1. [문장 단위 줄바꿈 및 해시태그 세로 나열]:
- 제목, 촬영 가이드, 본문 원고 텍스트 등 블로그 원고의 모든 텍스트 영역에서 문장마다 반드시 엔터(줄바꿈)를 치세요.
- 최하단 [필수 해시태그] 영역에서는 각 해시태그마다 반드시 엔터를 쳐서 세로로 한 줄씩 나열하세요.

2. [가이드 필수 내용 100% 반영 & 분량 기준]:
...
[원고 텍스트]
(가이드 필수 내용 및 30대 찐후기 반영 / 문장마다 엔터 쳐서 줄바꿈 / 공백 제외 1500~2000자 목표 분량 준수)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [가이드 필수 사진/문구/내용 100% 누락 없이 반영], [문장마다 엔터 쳐서 줄바꿈], [해시태그 무조건 세로로 한 줄씩 나열], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
