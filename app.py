...
5. [비포애프터(B/A) 자막 강제 삽입]:
- 장면 설명에 비포, 애프터, B/A, 비교 등의 키워드가 포함되어 있다면, 자막 영역에 반드시 독립된 줄로 `(비포애프터)`를 추가하세요.

6. [나레이션만 정리, 캡션, 해시태그 줄바꿈 규칙]:
- [나레이션만 정리] 영역은 각 씬별 나레이션을 반드시 엔터(줄바꿈)를 쳐서 세로로 한 줄씩 독립되게 나열하세요.
- [#광고 캡션] 영역은 문장마다 반드시 엔터를 쳐서 여러 줄로 나누어 작성하세요. (한 문단 뭉침 절대 금지)
...
2. [사진 장수 15장~20장 확보 & 사진 설명 바로 아래 본문 원고 1:1 매칭]:
...
(동일한 구조로 [사진 4]부터 [사진 15~20]까지 가이드와 본문 원고를 1:1로 끝까지 작성)
...
"""
                prompt_text = f"""
다음 정보를 바탕으로 위 블로그 템플릿 규칙([카테고리: {current_cat}], [기계식 인사말 절대 금지 및 자연스러운 후기 톤], [총 15장~20장의 풍성한 사진 컷 구성], [사진 설명 바로 밑에 해당 본문 원고 1:1 밀착 매칭], [본문 원고는 문장마다 엔터 쳐서 줄바꿈], [해시태그 무조건 세로로 한 줄씩 나열], [가이드 필수 내용 100% 반영], [공백 제외 글자수 1500~2000자 내외], [SEO 25~35자 제목], [시술명 금지])을 100% 지켜 네이버 블로그 원고를 작성해줘:
- 카테고리: {current_cat}
- 브랜드명: {brand_name}
- 제품명: {product_name}
- 제품 USP: {product_usp}
